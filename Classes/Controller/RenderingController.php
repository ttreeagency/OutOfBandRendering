...
class RenderingController extends ActionController {
     ...
     * @throws Exception
     */
    public function showAction($rendering) {
        /** @var NodeInterface $renderingNode */
        $renderingNode = $this->propertyMapper->convert($rendering['node'], 'TYPO3\TYPO3CR\Domain\Model\NodeInterface');
        ...
        $this->view->assign('value', $renderingNode);
        $presetDefinition = $this->presetDefinitionFactory->create($rendering['preset'], $renderingNode);
        $this->view->setTypoScriptPath($presetDefinition->getTypoScriptPath($renderingNode));
    }
